...
shaka.media.SimpleAbrManager.prototype.chooseVideoTrack_ = function() {
  ...
  if (window['PalmSystem']) {
    ...
    var maxWidth = devInfo['screenWidth'];
    var maxHeight = devInfo['screenHeight'];
    // take the minimal resolution if all tracks have higher resolution
    if (videoTracks[0].width > maxWidth || videoTracks[0].height > maxHeight) {
      maxWidth = videoTracks[0].width;
      maxHeight = videoTracks[0].height;
    }
    for (var i = 1; i < videoTracks.length &&
      videoTracks[i].width <= maxWidth &&
      videoTracks[i].height <= maxHeight;
      ++i) {
    }
    if (i < videoTracks.length) {
      videoTracks = videoTracks.slice(0, i);
    }
  }
