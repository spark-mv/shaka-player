...
shaka.abr.SimpleAbrManager.prototype.chooseAudioStream_ = function() {
  ...
  if (!audioStreamSet)
    return null;
  var audioStreams = SimpleAbrManager.sortStreamsByBandwidth_(
      audioStreamSet.streams);
...
shaka.abr.SimpleAbrManager.prototype.chooseVideoStream_ = function() {
  ...
  if (!videoStreamSet)
    return null;
  var videoStreams = videoStreamSet.streams;
  if (window['PalmSystem']) {
    // filter out tracks with resolution higher than the device supports
    var devInfo = JSON.parse(window['PalmSystem']['deviceInfo']);
    var devResolution = devInfo['screenWidth'] * devInfo['screenHeight'];
    var minResolution = Infinity;
    for (var i = 0; i < videoStreams.length; ++i) {
      var resolution = videoStreams[i].height * videoStreams[i].width;
      minResolution = Math.min(minResolution, resolution);
    }
    // take the minimal resolution if all tracks have resolution higher than
    // the device
    devResolution = Math.max(devResolution, minResolution);
    videoStreams = videoStreams.filter(function(s) {
      return s.width * s.height <= devResolution;
    });
    videoStreams = SimpleAbrManager.sortStreamsByBandwidth_(videoStreams);
  } else {
    videoStreams = SimpleAbrManager.sortStreamsByBandwidth_(
        videoStreamSet.streams);
  }
 ...
 * @private
 */
shaka.abr.SimpleAbrManager.sortStreamsByBandwidth_ = function(streams) {
  return streams.slice(0)
      .filter(function(s) {
